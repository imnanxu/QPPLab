...
The input file `data`.mat is included in ./input/ folder whereas `data` is the input filename. The input `data`.mat must include the following four variables.
...
In the ./input/ folder, 5 distinct input samples are encloused, which includes 1) a visual stimuated human brain dataset, 2) a Human Connectome Project resting human brain dataset, 3) a resting rat brain dataset, and 4) a resting mice brain dataset.

### 1.3 (optional) Input Variables Generation
