# QPPLab: A generally applicable MATLAB package for detecting, analyzing, and visualizing quasiperiodic spatiotemporal patterns (QPPs) of whole brain activity
...
### 1.2 Input File (./data/`data`.mat)
<a name="section-1-2"></a>
The input file `data`.mat is included in ./data/ folder whereas `data` is the input filename. Two distinct input samples are enclosed in the current ./data/, which includes a visually stimulated brain dataset, and a resting human brain dataset.
...
<a name="section-1-2-2"></a>
The `ROI2Net` and `NetLB` variables can be generated by executing 'st0_ROIreOrg.m' found in the ./code/ folder. Ensure that your original input file includes the EPI data cell matrix. If MotionInf parameter was not prepared, 'st0_ROIreOrg.m' will include all timepoints from each scan in MotionInf. You can disregard the warning message "Variable 'MotionInf' not found." in this situation.

In addition, you will also need to prepare an atlas-network file. This file should be a spreadsheet including at least two columns, one column including the numerical label of ROIs/Parcels corresponding to your atlas, and the other column including the network name each ROI belongs to. In ./asset/ folder, an examplary atlas-network file is included.

<a name="section-2"></a>
## 2. Main Pipeline
The main pipeline comprises three steps, with scripts located in the ./code/ folder. Specifically, the first step (st1_ParamSt.m) initializes the parameters. The second step (st2_QPPanalysis.m) identifies and analyzes QPPs. The third step (st3_QPPFCvisual.m) visualizes QPPs and related results using the outputs from step 2. Note: When running the pipeline in Matlab, ensure that all scripts are executed within the ./code/ folder.
