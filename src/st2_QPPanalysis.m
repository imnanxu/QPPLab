...
p2param=['Params_' dataext '.mat']; load(['../params/' p2param]); addpath(p2qppf);
...
d2O='../results/'; if ~exist(d2O,'dir'), mkdir(d2O); end % directory to outputs files
if runM==1
...
%% Computation
delete(gcp('nocreate'));
myCluster = parcluster('local'); myCluster.NumWorkers = 12;  % 'Modified' property now TRUE
